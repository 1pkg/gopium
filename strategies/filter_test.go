...
func TestFilter(t *testing.T) {
	...
	table := map[string]struct {
		...
		"non empty struct should be applied accordingly to mixed filter": {
			filter: filter{nregex: regexp.MustCompile(`^test-2$`), exp: &fvar},
			ctx:    context.Background(),
			o: gopium.Struct{
				Name: "test",
				Fields: []gopium.Field{
