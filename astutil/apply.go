...
var FFN = combine(
	ufmt(fmtio.FSPT),
	filter,
	note(goparse, fmtio.Goprint(0, 4, false)),
)

// combine helps to pipe several
// ast helpers to single apply func
func combine(funcs ...Apply) Apply {
	return func(ctx context.Context, pkg *ast.Package, l gopium.Locator, h collections.Hierarchic) (*ast.Package, error) {
...
func ufmt(fmt fmtio.Ast) Apply {
	return func(ctx context.Context, pkg *ast.Package, l gopium.Locator, h collections.Hierarchic) (*ast.Package, error) {
		// just reuse inner walk helper
		wpkg, err := walk(ctx, pkg, compid(l, h), wact(fmt))
		if err != nil {
			return nil, err
		}
...
func filter(ctx context.Context, pkg *ast.Package, l gopium.Locator, h collections.Hierarchic) (*ast.Package, error) {
...
func note(p parse, pr fmtio.Printer) Apply {
	return func(ctx context.Context, pkg *ast.Package, l gopium.Locator, h collections.Hierarchic) (*ast.Package, error) {
		// create sync error group
		// with cancelation context
		...
		for name, file := range pkg.Files {
			...
			group.Go(func() error {
				...
				if err := pr(&buf, l.Root(), file); err != nil {
					return err
				}
				// parse ast back to file
				// and push child fset to locator
				fset := token.NewFileSet()
				file, err := p(fset, buf.String(), parser.ParseComments|parser.AllErrors)
				if err != nil {
					return err
				}
				l.Fset(name, fset)
